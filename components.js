...
let menuBtnArr = document.getElementsByClassName("menu-btn-btn");
let menuBtnMenuArr = document.getElementsByClassName("menu-btn-menu");

for (let i = 0; i < menuBtnArr.length; i++) {
    menuBtnArr[i].addEventListener("click", function () {
        menuBtnArr[i].classList.toggle("menu-open");
        menuBtnMenuArr[i].classList.toggle("menu-open");
    })
}